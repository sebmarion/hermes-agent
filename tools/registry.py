...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # checked-in builtin entries retained for the bounded planning sandbox,
        # independent of any plugin overlay in ``_scoped_tools``.
    ...
    def _snapshot_state(
        ...
        with self._lock:
            if _bestplan_builtin_only():
                entries = list(self._builtin_tools.values())
            else:
    ...
    def snapshot_registration(
        ...
        *,
        ...
        with self._lock:
            target = (
                self._builtin_tools
                if scope is None and _bestplan_builtin_only()
                else (self._tools if scope is None else self._scoped_tools.get(scope, {}))
            )
