...
def _finalize_child_results(
    ...
    parent_agent, *, apply_summary_budget: bool = True,
) -> None:
...
def _run_child_lifecycle(task_index: int, goal: str, child=None, parent_agent=None, *, apply_summary_budget: bool = True) -> Dict[str, Any]:
    ...
    task = {"goal": goal}
    _finalize_child_results([result], [{"goal": ""} for _ in range(task_index)] + [task], [(task_index, task, child)], parent_agent, apply_summary_budget=apply_summary_budget)
    return result
