...
# below the total window so a local model has room to produce a useful result.
...
def _build_child_agent(
    ...
    # Credential inheritance is a separate trust boundary from model fallback.
    # Explicit lanes must not send the parent's key/base URL to a different or
    # incomplete endpoint merely because the lane omitted its own credential.
    ...
    # Resolve effective credentials. Explicit lane callers disable parent
    # credential inheritance so a lane with its own provider/base URL cannot
    # receive the parent's secret or endpoint by omission.
    ...
    # Share a credential pool with ordinary children so they can rotate on rate
    # limits. Explicit lanes own their credential boundary: attaching any pool
    # could replace the selected key/endpoint during _run_single_child().
    child_pool = (
...
def _bound_child_handoff(
    *,
    ...
    separator = "\n\n[... middle omitted ...]\n\n"
    footer_text = "\n\n" + "\n".join(footer)
    # Leave a small deterministic margin because the final "Showing ..."
    # counters can be a few characters longer than the sizing placeholder.
        ...
        for i, (t, lane_name, task_creds, lane_toolsets) in enumerate(task_specs):
            ...
            try:
                ...
                for _, _, built_child in children:
                    # _build_child_agent registers and announces before returning.
                    # A later construction failure must fully balance that state.
                    if hasattr(parent_agent, "_active_children"):
                        try:
                            lock = getattr(parent_agent, "_active_children_lock", None)
                            if lock:
...
def _apply_local_first_lane_policy(
    lane: str,
    cfg: dict,
    lanes: dict,
) -> str:
    """Fail closed from the local execution lane unless controller state is healthy.

    The controller owns the durable health decision. Routing deliberately treats
    missing, unreadable, oversized, malformed, stale, or non-``local`` state as
    a degraded condition. Only the configured local execution lane is overlaid;
    adversary, reasoning, and frontier routes remain explicit.
    """
    ...
    if not isinstance(policy, dict):
        raise ValueError(
            "delegation.local_first must be a mapping when configured."
        )
    ...
    raw_max_age = policy.get("max_state_age_seconds", 1800)
    try:
        max_state_age = int(raw_max_age)
    ...
    if (
        state.get("mode") != "local"
        or state_age > max_state_age
        or state_age < -300
    ):
        return degraded_lane
    return local_lane
...
def _resolve_lane_for_task(task: dict, cfg: dict) -> Optional[str]:
    ...
    delegation route; missing configuration fails closed.
    """
    ...
    if not lanes:
        ...
        if (
            mode == "execute"
            and isinstance(explicit_provider, str)
...
def _resolve_delegation_credentials_for_task(
    ...
    if task is not None:
        ...
        if lane_name == "code_worker":
            # Compatibility for older installs that explicitly configured a
