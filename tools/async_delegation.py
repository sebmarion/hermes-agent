...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    columns = {row[1] for row in conn.execute("PRAGMA table_info(async_delegations)")}
    for name, kind in (("child_session_id", "TEXT"), ("launch_id", "TEXT"),
                       ("origin_version", "INTEGER"), ("created_session_id", "TEXT")):
        if name not in columns:
            conn.execute(f"ALTER TABLE async_delegations ADD COLUMN {name} {kind}")
...
def _restore_undelivered_completions_current_profile(target_queue) -> int:
...
def _event_delivery(fn, evt: Dict[str, Any], claim_id: str) -> bool:
    if claim_id and evt.get("type") == "async_delegation":
        return fn(str(evt.get("delegation_id") or ""), claim_id)
    return True
...
def _dispatch(
    *, delegation_id: str, goal: str, goals: Optional[List[str]], context: Optional[str],
    ...
    try:
        ...
        # INSERT may have rejected a duplicate; never delete that existing row.
        raise
    try:
        if on_registered is not None:
            on_registered(delegation_id)
        executor = _get_executor(max_async_children)
    except Exception:
        with _records_lock:
            _records.pop(delegation_id, None)
        with _DB_LOCK, _transaction() as conn:
            conn.execute("DELETE FROM async_delegations WHERE delegation_id=? AND state='running'", (delegation_id,))
        raise
...
def dispatch_async_delegation_batch(
    *, goals: List[str], context: Optional[str], toolsets: Optional[List[str]], role: str, model: Optional[str],
    ...
    handle = _dispatch(
        ...
        origin_ui_session_id=origin_ui_session_id, origin_session_id=origin_session_id,
        interrupt_fn=interrupt_fn, max_async_children=max_async_children, progress_fn=progress_fn,
        on_registered=on_registered, capacity_error=(
            f"Async delegation capacity reached ({max_async_children} running). Wait for one to finish "
...
def _finalize(delegation_id: str, result: Any, status: str) -> None:
    """Keep native single/batch/stall semantics, retry the exact terminal payload."""
    claimed = _begin_finalization(delegation_id)
    if claimed is None:
        return
    snapshot, _interrupt_fn = claimed
    payload = result(snapshot) if callable(result) else result
    _publish_finalization_with_retry(
        delegation_id, status, lambda: _push_completion_event(snapshot, payload, status))
...
def _push_completion_event(record: Dict[str, Any], result: Dict[str, Any], status: str) -> None:
    ...
    try:
        ...
        logger.error(f"Async delegation{label} %s finished but process_registry import failed; "
                     "publication deferred: %s", record.get("delegation_id"), exc)
...
from tools.async_delegation_outbox import (_has_persisted_terminal_assistant, ack_terminal_output, bind_child_delegation, claim_terminal_outputs, commit_terminal_output, event_delivery_claim_status, get_delivery_receipt, list_durable_delegations, list_terminal_outputs, mark_terminal_output_live_published)

def restore_undelivered_completions(target_queue, *, profile_home=None) -> int:
    from hermes_constants import set_hermes_home_override, reset_hermes_home_override
    token = set_hermes_home_override(profile_home) if profile_home else None
    try:
        return _restore_undelivered_completions_current_profile(target_queue)
    finally:
        if token is not None:
            reset_hermes_home_override(token)


from tools.async_delegation_outbox import renew_completion_delivery

from tools.async_delegation_outbox import renew_event_delivery

from tools.async_delegation_outbox import replay_terminal_output
...
def _publish_finalization_with_retry(
    ...
    def _retry() -> None:
        while True:
            with _records_lock:
                if (_records.get(delegation_id) or {}).get("status") != "finalizing":
                    return
            if _attempt():
                return
    ...
    threading.Thread(
        target=propagate_context_to_thread(_retry),
...
def _push_batch_completion_event(record: Dict[str, Any], result: Dict[str, Any], status: str) -> None:
    """Compatibility name sharing the native batch event construction."""
    _push_completion_event({**record, "is_batch": True}, result, status)
