...
# verify commands (e.g. ``test:unit``, ``lint:css``).  This lets projects with
# namespaced script names produce verification events.
    ...
    if (root / "package.json").is_file():
        ...
        for name in scripts:
            ...
            for target in _VERIFY_TARGETS:
                for separator in _VERIFY_PREFIX_SEPARATOR:
                    if name.startswith(target + separator):
                        prefixed_by = target
                        break
                if prefixed_by:
                    break
        ...
        exact = [name for name in collected if name in _VERIFY_TARGETS]
        prefixed = [name for name in collected if name not in _VERIFY_TARGETS]
