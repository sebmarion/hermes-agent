...
def auto_title_session(
    ...
    Called on a background thread. If the LLM fails or returns no usable
    title, falls back to a whitespace-collapsed, at-most-80-character summary
    of the first user message (exact Hermes skill scaffolding is summarized by
    the canonical skill-invocation parser). Silently skips if:
    - session_db is None
...
def _auto_title_session(
    ...
    if not title:
        ...
        if is_titleable_user_message(user_message):
            title = derive_title(user_message)
        else:
