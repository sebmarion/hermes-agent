...
BESTPLAN_RECEIPT_START = "<<<HERMES_BESTPLAN_RECEIPT_V1>>>"
BESTPLAN_RECEIPT_END = "<<<END_HERMES_BESTPLAN_RECEIPT_V1>>>"
...
_RECEIPT_BLOCK_RE = re.compile(
    re.escape(BESTPLAN_RECEIPT_START)
    + r".*?"
    + re.escape(BESTPLAN_RECEIPT_END),
    re.DOTALL,
)
...
def _safe_process_start_token(pid: int) -> str:
    try:
...
def _dispatch_owner_identity() -> str:
    pid = os.getpid()
    token = _safe_process_start_token(pid)
    if not token:
        raise RuntimeError("BestPlan dispatch owner identity is unavailable")
    return json.dumps(
...
def _dispatch_owner_is_live(value: Any) -> bool:
    try:
        owner = json.loads(str(value or ""))
        pid = owner.get("pid")
        ...
        if (
            ...
            return False
        return _safe_process_start_token(pid) == token
    except (AttributeError, TypeError, ValueError, json.JSONDecodeError):
        return False


def _runtime_owner_is_live(record: Any) -> bool:
    try:
        if not isinstance(record, dict):
            return False
        pid = record.get("runtime_owner_pid")
        token = record.get("runtime_owner_start_token")
        if (
            isinstance(pid, bool)
            or not isinstance(pid, int)
            or pid <= 1
...
def _strip_bestplan_envelope(value: Any) -> str:
    visible = _RECEIPT_BLOCK_RE.sub("", str(value or ""))
    receipt_start = visible.find(BESTPLAN_RECEIPT_START)
    if receipt_start >= 0:
        visible = visible[:receipt_start]
    visible = visible.replace(BESTPLAN_RECEIPT_END, "")
    visible = _ENVELOPE_BLOCK_RE.sub("", visible)
...
def sanitize_bestplan_response_for_external_sink(value: Any) -> str:
    """Return user-visible text without exposing an internal plan envelope."""
    text = str(value or "")
    if (
        BESTPLAN_ENVELOPE_START not in text
        and BESTPLAN_ENVELOPE_END not in text
        and BESTPLAN_RECEIPT_START not in text
        and BESTPLAN_RECEIPT_END not in text
    ):
        return text
    return _strip_bestplan_envelope(text) or (
        "[BestPlan plan envelope withheld pending host validation.]"
    )
    ...
    for item in messages:
        ...
        if (
            ...
            and (
                ...
                or BESTPLAN_ENVELOPE_END in content
                or BESTPLAN_RECEIPT_START in content
                or BESTPLAN_RECEIPT_END in content
            )
        ):
            item["content"] = sanitize_bestplan_response_for_external_sink(
                content
            )
            ...
            for block in content:
                ...
                for key in ("text", "content"):
                    ...
                    if (
                        ...
                        and (
                            BESTPLAN_ENVELOPE_START in value
                            or BESTPLAN_ENVELOPE_END in value
                            or BESTPLAN_RECEIPT_START in value
                            or BESTPLAN_RECEIPT_END in value
                        )
                    ):
                        block[key] = (
                            sanitize_bestplan_response_for_external_sink(value)
                        )
...
class BestplanStore:
    ...
    def __init__(self, session_db=None, db_path: Optional[Path] = None):
        ...
        if self._session_db is None and self._db_path is not None:
            self._prepare_private_sqlite_path()
            ...
            self._harden_private_sqlite_files()
        self._ensure_schema()
        self.reconcile_async_tracker()

    def _prepare_private_sqlite_path(self) -> None:
        ...
        if (
            ...
            raise RuntimeError("BestPlan authority directory is unsafe")
        os.chmod(parent, 0o700)
        if self._db_path.exists() or self._db_path.is_symlink():
            state = self._db_path.lstat()
            if (
                self._db_path.is_symlink()
                or not stat.S_ISREG(state.st_mode)
                or state.st_uid != os.geteuid()
                or state.st_nlink != 1
            ):
                raise RuntimeError("BestPlan authority database is unsafe")
            os.chmod(self._db_path, 0o600)

    def _harden_private_sqlite_files(self) -> None:
        ...
        for path in (
            ...
            if (
                ...
                raise RuntimeError("BestPlan SQLite artifact is unsafe")
            os.chmod(path, 0o600)
        ...
        def reconcile(conn):
            ...
            for row in rows:
                ...
                if phase in {"scheduled", "running"}:
                    owner_live = _runtime_owner_is_live(record)
                    if owner_live:
    ...
    def begin_dispatch_attempt(self, plan_id: str) -> bool:
        now = time.time()
        owner = _dispatch_owner_identity()
        return bool(self._execute_write(lambda conn: conn.execute(
            ...
            (owner, now, now, plan_id, PlanState.RUNNING),
        ).rowcount))
    ...
    def recover_dead_dispatch_owners(self) -> int:
        def recover(conn):
            ...
            for row in rows:
                ...
                if _dispatch_owner_is_live(owner):
                    continue
                changed += conn.execute(
