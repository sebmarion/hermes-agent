...
_REQUEST_REJECTIONS: tuple[tuple[Callable[[Any], bool], str], ...] = (
    ...
    (lambda r: r.timeout_seconds is not None and (
        isinstance(r.timeout_seconds, bool) or not isinstance(r.timeout_seconds, (int, float))
        or not math.isfinite(r.timeout_seconds) or r.timeout_seconds <= 0),
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        child = _build_child_preserving_parent_tools(
            ...
            model=model, override_provider=provider, override_api_mode=api_mode,
            override_base_url=base_url, override_api_key=api_key,
            max_iterations=DEFAULT_MAX_ITERATIONS, task_count=1, parent_agent=parent,
            role=request.role, run_budget_seconds=request.timeout_seconds,
        )
    ...
    def _run(self, record: _Record, goal: str, parent: Any) -> None:
        ...
        try:
            ...
            raw = _run_child_lifecycle(0, goal, record.agent, parent, apply_summary_budget=False)
