...
_KANBAN_DIR_RE = re.compile(r"^(.*[/\\]\.worktrees)[/\\]t_[0-9a-f]+[/\\]?$", re.IGNORECASE)


def _branch_lane_id(repo_root: str, branch: str = "") -> str:
    """Desktop-compatible lane id for main-checkout branch buckets."""
...
def _kanban_worktree_dir(path: str) -> str | None:
    match = _KANBAN_DIR_RE.match(path or "")
...
def classify_session_workspace(
    ...
    *,
    ...
    if repo_root and wt_path:
        ...
        kanban_dir = _kanban_worktree_dir(wt_path)
        if kanban_dir:
            worktree_key = _kanban_lane_id(repo_root)
            worktree_label = "kanban"
            wt_path = kanban_dir
            kind = "worktree"
        elif wt_path == repo_root:
            worktree_label = branch or DEFAULT_BRANCH_LABEL
            worktree_key = _branch_lane_id(repo_root, worktree_label)
            kind = "directory"
        else:
