        ...
        *compression_lock_holder* refreshes the lease on the same ``conn`` before the expiry check (no
        ...
        def _do(conn):
            ...
            conn.execute(
                ...
                "'$._compression_from',parent_session_id) WHERE id=? AND parent_session_id IS NOT NULL "
                "AND json_extract(COALESCE(model_config,'{}'),'$._compression_from') IS NULL", (child_session_id,))
