...
class PluginContext:
    ...
    def subagent_lifecycle(self) -> Any:
        """Return the public, host-owned subagent lifecycle service."""
    ...
    def register_middleware(self, kind: str, callback: Callable) -> PluginRegistration:
        ...
        plugin_key = self.manifest.key or self.manifest.name
        registration = PluginMiddlewareRegistration(
            plugin_key=plugin_key,
            kind=kind,
            callback=callback,
        )
        ...
        handle = self._track(
            "middleware", kind,
            lambda: self._manager._remove_callback(
                self._manager._middleware, kind, registration
            ),
        )
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        """
        with _required_policy_recovery_lock, self._discovery_lock, \
                _plugin_home_scope(self.home_path):
            if self._discovered and not force:
                return
            discovery_home = str(_resolved_hermes_home())
...
def invoke_hook(hook_name: str, **kwargs: Any) -> List[Any]:
    ...
    # plugin code. Route through _delivery_manager() so surfaces that never ran
    # explicit discovery still fire (tracking #64178 — salvaged from PR #64188).
    with _required_policy_recovery_lock:
...
def has_hook(hook_name: str) -> bool:
    """Return True when a plugin handles a hook.
