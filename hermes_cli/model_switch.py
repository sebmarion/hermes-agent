...
def resolve_persist_behavior(
    ...
    5. Otherwise the switch is session-only.  The legacy
       ``model.persist_switch_by_default`` setting is intentionally ignored;
       persistence must be an explicit, visible ``--global`` choice.
    """
    if is_once:
        return False
    if is_session:
