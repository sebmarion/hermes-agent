    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 21:
                # v21 reconciles two independent v20 histories: one shipped
                # the activity projection while the other shipped per-model
                # usage attribution. Run both idempotent data migrations so a
                # database opened by either v20 lineage receives the missing
                # half before its schema version advances.
                #
                # Activity is deliberately backfilled by a daemon after this
                # startup transaction commits. Marking the resumable cursor
                # here is O(1); no message aggregation occurs on the
                ...
                # Going forward update_token_counts() records each API call
                # into session_model_usage keyed by the live model. Existing
                # sessions only have aggregate totals on the sessions row, so
                # seed one historical row for consumers such as Insights.
                # INSERT OR IGNORE makes this safe for the usage-aware v20
                # lineage and prevents stale aggregates from being doubled.
                try:
                    cursor.execute(
