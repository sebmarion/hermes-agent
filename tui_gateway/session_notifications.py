...
def _notif_dispatch_event(sid: str, session: dict, evt: dict, text: str) -> None:
    """Deliver a claimed completion without acknowledging merely scheduled work."""
    from tools.async_delegation import claim_event_delivery, complete_event_delivery, release_event_delivery
    home = session.get("profile_home")
    asynchronous = evt.get("type") == "async_delegation"
    heartbeat = None
    try:
        ...
        _notif_release_turn(session)
        if asynchronous:
            _requeue_async_delegation(evt, home) if home else _requeue_async_delegation(evt)
        return False
    if claim is None:
        _notif_release_turn(session)
        if asynchronous:
    ...
    if asynchronous:
        heartbeat = _start_async_delegation_heartbeat(
            evt, claim, profile_home=home, on_lost=lambda: _fail_closed_async_delivery_claim(session))
        kwargs["terminal_callback"] = _async_delegation_terminal_callback(
            evt, claim, heartbeat=heartbeat, profile_home=home)
    try:
    ...
    if accepted is False:
        _notif_release_turn(session)
        if asynchronous and getattr(kwargs.get("terminal_callback"), "_owner_terminal_committed", False):
            return
        if heartbeat:
            _stop_async_delegation_heartbeat(*heartbeat)
        def release():
            return _profile_scoped_async_db_call(home, lambda: release_event_delivery(evt, claim)) is not False
        def requeue():
            _requeue_async_delegation(evt, home) if home else _requeue_async_delegation(evt)
        try:
            released = release()
            ...
            released = False
        if released:
            if asynchronous: requeue()
        elif asynchronous:
            _schedule_async_delivery_transition_retry(evt, release, heartbeat=None, on_success=requeue, profile_home=home)
        return False
    if not asynchronous:
...
def _notif_handle_event(sid, session, evt, emitted, registry, fmt, deferred, attempted=None) -> bool:
    ...
    if deferred is not None and attempted is not None and id(evt) in attempted:
        # A failed normal-loop attempt just requeued this exact event. Shutdown
        # must hand it back, not immediately dispatch/release it a second time.
        if evt.get("type") != "async_delegation" or _async_delegation_delivery_is_retryable(evt):
            deferred.append(evt)
        return True
...
def _notification_poller_loop(stop_event: threading.Event, sid: str, session: dict) -> None:
    ...
    handle = lambda evt, deferred: _notif_handle_event(  # noqa: E731
        sid, session, evt, emitted, process_registry, format_process_notification, deferred, attempted)
...
def _async_delegation_display_metadata(evt: dict) -> dict:
    ...
    return {"delegation_id": str(evt.get("delegation_id") or ""),
            "delivery_id": "async-delegation:" + str(evt.get("delegation_id") or ""), "task_count": task_count,
