...
def _run_after_agent_ready(rid, sid, session, text, display_kind, hosted_terminal_callback):
    ...
    if err:
        ...
        with session["history_lock"]:
            for key in ("_owner_admit_callback", "_owner_action_id", "_owner_request_id", "_chief_automation"):
    ...
    with session["history_lock"]:
        ...
        if owner_admit is not None:
            if not callable(owner_admit):
                return _err(rid, 4003, "invalid owner dispatch proof"), fields
            if session.get("running"):
                return _err(rid, 4091, "owner task is busy"), fields
            if _ensure_session_db_row(session) is False:
                return _err(rid, 5072, "session storage unavailable; owner reply was not admitted"), fields
