...
class WSTransport:
    ...
    def __init__(self, ws: Any, loop: asyncio.AbstractEventLoop, *, peer: str = "unknown",
                 auth_identity: dict | None = None, auth_scope: dict | None = None) -> None:
        self._ws = ws
        self._loop = loop
        self._peer = peer
...
async def handle_ws(ws: Any, *, auth_identity: dict | None = None, auth_scope: dict | None = None, subprotocol: str | None = None) -> None:
    ...
    try:
        ...
        transport = WSTransport(ws, asyncio.get_running_loop(), peer=peer, auth_identity=auth_identity, auth_scope=auth_scope)
