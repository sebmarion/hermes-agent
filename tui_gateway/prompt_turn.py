        ...
        for index, (_evt, synth) in enumerate(drained):
            with session["history_lock"]:
                if session.get("running"):
                    ...
                    break
                session["running"] = True
            # Use the poller's exact durable-terminal callback, never ACK a
            # completion merely because a follow-up thread was scheduled.
...
class _TurnRun:
    ...
    marker_retired: bool = False
    terminal_published: bool = False
    skip_terminal_publication: bool = False
...
def _complete_turn_payload(session: dict, st: _TurnRun, status_note: str | None, cols: int):
    ...
    if status == "complete" and (not isinstance(raw, str) or raw.strip() in {"", "(empty)"}):
...
def _recover_turn_exception(sid: str, session: dict, st: _TurnRun, e: BaseException) -> None:
    ...
    if getattr(st.terminal_callback, "_is_async_delegation_terminal_callback", False):
        # The durable outbox, once committed, owns this answer. Cleanup failure
        # must not publish another terminal error or replay the model turn.
        if st.receipt_committed:
            logger.warning("cleanup failed after durable terminal output", exc_info=True)
            return
        if not st.receipt_attempted:
            try:
                _settle_owner_terminal_error(sid, session, st, e)
                return
            except Exception:
                logger.warning("terminal outbox failure remains pending", exc_info=True)
        st.error_retained = True
        st.error_detail = str(type(e).__name__)
        return
    import traceback
    with contextlib.suppress(Exception):
...
def _run_prompt_submit(
    ...
    def run():
        ...
        st.runtime_session_id = sid
        try:
            st.marker_key = _record_turn_marker(session, text)
        except Exception as setup_error:
            try:
                _settle_owner_terminal_error(sid, session, st, setup_error)
            finally:
                with session["history_lock"]:
                    session["running"] = False
        ...
        try:
            ...
            if prepared is None:
                # Input validation already emitted an error. Store a durable
                # blocker without publishing a second terminal frame.
                _settle_owner_terminal_error(sid, session, st,
                    RuntimeError("Turn blocked before model execution; delegated output remains available."),
                    publish=False)
                return
            ...
            if (getattr(st.terminal_callback, "_is_async_delegation_terminal_callback", False)
                    and not st.receipt_attempted):
                with contextlib.suppress(Exception):
                    _settle_owner_terminal_error(sid, session, st, RuntimeError("Turn did not start; delegated output remains durable."))
