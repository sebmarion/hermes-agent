...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            ("POST", "/v1/runs/{run_id}/release-checkpoint", self._handle_release_checkpoint_steer),
    ...
    def _create_agent(
        ...
        ``request_reasoning_effort`` is a per-run override. ``ultra`` is a
        fail-closed GPT-5.6 Sol contract: it forces the Codex app-server
        runtime and disables provider fallback.
        """
        ...
        request_model = _clean_request_string(request_model or requested_model)
        request_provider = _clean_request_string(requested_provider)
        ...
        if is_sol_ultra:
            normalized_model = str(requested_model or model or "").strip().lower()
        ...
        fallback_model = (
            None
            if confirmed_runtime_lock or is_sol_ultra
            ...
            try:
                ...
                with self._profile_scope(request_profile):
                    agent = self._create_agent(
                        ...
                        gateway_session_key=gateway_session_key,
                        requested_model=agent_overrides.get("requested_model"),
                        request_model=agent_overrides.get("requested_model"),
                        requested_provider=agent_overrides.get("requested_provider"),
