...
from functools import lru_cache as _boot_identity_cache

@_boot_identity_cache(maxsize=1)
def _gateway_boot_source_digest():
    from gateway.code_skew import runtime_source_digest
    from pathlib import Path
    return runtime_source_digest(Path(__file__).resolve().parents[1])
