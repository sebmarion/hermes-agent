...
def test_deterministic_batch_dispatch_id_is_idempotent(tmp_path):
    ...
    tracker.parent.mkdir(mode=0o700)
    ...
    ad._persist_record(
        {
            ...
            "runtime_owner_id": "dead-runtime",
            "runtime_owner_pid": 99_999_999,
            "runtime_owner_start_token": "dead-start-token",
...
def test_scheduled_pid_reuse_is_retryable_intent(tmp_path, monkeypatch):
    tracker = tmp_path / "profile" / "async_delegations.json"
    tracker.parent.mkdir(mode=0o700)
    delegation_id = "bestplan-stale-scheduled-owner"
    ad._persist_record(
        {
            "delegation_id": delegation_id,
            "status": "scheduled",
            ...
            "owner_pid": os.getpid(),
            "runtime_owner_id": "old-runtime",
            "runtime_owner_pid": os.getpid(),
            "runtime_owner_start_token": "stale-start-token",
            "origin_tracker_path": str(tracker),
            "origin_profile": "coder",
            "session_key": "s",
        },
    ...
    monkeypatch.setattr(
        ad, "_safe_process_start_token", lambda _pid: "current-start-token"
    )

    recovered = ad.recover_async_delegations(tracker)
    assert recovered == {"lost": 0, "queued": 0}
    persisted = ad._read_persisted_unlocked(tracker)["records"][delegation_id]
    assert persisted["status"] == "intent"
    assert persisted["record"]["status"] == "intent"
...
def test_delegate_task_background_routes_async_and_does_not_block(monkeypatch):
    ...
    fake_child._delegate_lane = "code_worker"
    fake_child._delegate_provider = "test"
    fake_child._delegate_model = "m"
...
def test_delegate_task_background_detaches_child_from_parent(monkeypatch):
    ...
    fake_child._delegate_provider = "test"
    fake_child._delegate_model = "m"
    fake_child._delegate_mode = "execute"
    fake_child._subagent_id = "s1"
