...
def test_persist_git_metadata_async_skips_invalid_input(tmp_path):
    assert (
        persist_git_metadata_async(
            db_path=tmp_path / "state.db", session_id="", cwd=tmp_path
        )
        is None
    )
    assert (
        persist_git_metadata_async(
            db_path=tmp_path / "state.db", session_id="session", cwd="relative"
        )
        is None
    )
    assert (
        persist_git_metadata_async(
            db_path=MagicMock(), session_id="session", cwd=tmp_path
        )
        is None
    )
