...
def test_shared_budget_exhaustion_is_not_reported_completed(monkeypatch):
    """A shared budget stop is incomplete even below the local call ceiling."""
    monkeypatch.setattr("hermes_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    agent = _LimitAgent(max_iterations=60, budget_remaining=0)
...
def test_pending_response_does_not_release_parent_task_outside_dispatcher_context(
    monkeypatch,
):
    ...
    with non_dispatcher_owned_context():
        result = _finalize(
            ...
            final_response=None,
            exit_reason="unknown",
            pending_verification_response="composed child report",
        )
