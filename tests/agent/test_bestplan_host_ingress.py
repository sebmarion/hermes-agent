...
def test_capture_strips_machine_envelope_and_host_renders_authority(tmp_path):
    ...
    response = (
        ...
        '{"version":1,"body_sha256":"untrusted-for-host-authority"}\n'
    ...
    assert "HERMES_BESTPLAN_RECEIPT" not in capture.response
...
def test_recovery_reclaims_pid_reuse_with_wrong_start_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        bestplan_state, "_safe_process_start_token", lambda _pid: "live-token"
    )
    store._connection().execute(
        "UPDATE bestplan_plans SET dispatch_state='dispatching', dispatch_owner=? "
        "WHERE plan_id=?",
        (
            json.dumps(
                {"pid": os.getpid(), "start_token": "stale-token"},
                sort_keys=True,
                separators=(",", ":"),
            ),
            capture.plan_id,
...
def test_direct_bestplan_sqlite_authority_is_private_under_umask_022(tmp_path):
    authority = tmp_path / "profile" / "state.db"
    previous_umask = os.umask(0o022)
    try:
        store = BestplanStore(db_path=authority)
        _capture(store)
        artifacts = [
            authority,
            Path(str(authority) + "-wal"),
            Path(str(authority) + "-shm"),
        ]
