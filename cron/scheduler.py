...
class CronTickYielded(RuntimeError):
    ...
    def __init__(self, boot_rev: str, disk_rev: str) -> None:
        ...
        super().__init__(
            "Cron tick yielded to a fresh gateway process "
            f"(stale code: booted on {boot_rev}, disk is at {disk_rev})"
        )
...
def _log_tick_yield_once(reason: str) -> None:
    """Log a repeated stale-ticker yield at most once per hour."""
...
def _target_mirror_eligible(
    ...
    *,
    ...
    Origin targets remain eligible. An origin-less ``deliver=origin`` fallback
    is eligible under the same per-job/global flags, while an explicit target
    requires the job's own ``attach_to_session`` opt-in. Broadcast expansions
    carry no provenance and remain ineligible.
    """
...
def _inchannel_seed_allowed(*, is_dm: bool, user_id: Optional[str]) -> bool:
    """Whether a flat in-channel session seed can map back to a real session."""
    ...
    for part in parts:
        ...
        if target:
            ...
            if key not in seen:
                ...
                kept = seen[key]
                if _MIRROR_PROVENANCE_RANK.get(
                    str(target.get("_resolved_from") or ""), 0
                ) > _MIRROR_PROVENANCE_RANK.get(
                    str(kept.get("_resolved_from") or ""), 0
                ):
...
def _deliver_result(
    ...
    for target in targets:
        ...
        # Mirror only the origin, an origin-less home fallback, or an explicit
        # target whose job opted into transcript attachment. Broadcast targets
        # remain excluded.
        ...
        mirror_this_target = mirror_enabled and _target_mirror_eligible(
            job,
            target,
            global_mirror=mirror_enabled,
            origin_match=origin_target,
        )
        ...
        if transport is not None:
            ...
            if platform_name in relay_fronted_platforms():
                ...
                delivery_errors.append(msg)
                continue
            # Preserve standalone delivery for natively configured platforms.
                ...
                if adapter_ok:
                    ...
                    if (
                        ...
                        logger.warning(
                            ...
                            "continuable target (origin=%s:%s thread=%s) — seed "
                            "skipped; the plain mirror may still apply",
                            job["id"], platform_name, chat_id,
...
def _snapshot_process_create_time(proc: subprocess.Popen) -> Optional[float]:
    """Capture the spawned root identity; return ``None`` if it is not provable."""
    if proc.poll() is not None:
        return None
    try:
        import psutil
        from hermes_cli.process_identity import coerce_process_create_time

        create_time = coerce_process_create_time(psutil.Process(proc.pid).create_time())
    except Exception:
        return None
    if proc.poll() is not None:
        return None
...
def _terminate_cron_script_process(
    proc: subprocess.Popen, *, expected_create_time: Optional[float] = None
) -> None:
    ...
    if sys.platform == "win32":
        try:
            ...
            if not kill_process_tree(
                proc.pid, expected_create_time=expected_create_time
            ):
                proc.kill()
        except Exception:
            # ``Popen.kill`` uses the retained Windows process handle, so this
            # fallback cannot target a recycled PID. Tagged descendants are
            # swept separately by ``_terminate_cron_script_tree``.
            proc.kill()
    else:
        try:
...
_CRON_SCRIPT_RUN_TOKEN_ENV = "HERMES_INTERNAL_CRON_SCRIPT_RUN_TOKEN"


def _terminate_tagged_cron_script_processes(run_token: Optional[str]) -> bool:
    """Kill script processes carrying this invocation's unguessable token.

    Descendants inherit the token even after ``setsid`` and after their root
    exits and is reparented, when a parent/child walk can no longer find them.
    ``psutil.Process`` keeps PID/create-time identity bound through signalling.
    """
    if not run_token:
        return False
    try:
    ...
    for candidate in psutil.process_iter():
        try:
            if candidate.pid == os.getpid():
                continue
            if candidate.environ().get(_CRON_SCRIPT_RUN_TOKEN_ENV) == run_token:
...
def _sweep_tagged_cron_script_processes(run_token: Optional[str]) -> None:
    """Rescan after root teardown until no newly tagged descendant is found."""
    for _ in range(3):
        if not _terminate_tagged_cron_script_processes(run_token):
            return
...
def _terminate_cron_script_tree(
    ...
    *,
    ...
    _terminate_tagged_cron_script_processes(run_token)
    if proc.poll() is not None:
        ...
        _sweep_tagged_cron_script_processes(run_token)
        return
    ...
    if not isinstance(pid, int) or pid <= 0:
        ...
        _terminate_cron_script_process(
            proc, expected_create_time=expected_create_time
        )
        _sweep_tagged_cron_script_processes(run_token)
        return
    try:
        ...
        return
    try:
        if kill_process_tree(pid, expected_create_time=expected_create_time):
            _sweep_tagged_cron_script_processes(run_token)
            return
        logger.warning(
    ...
    _terminate_cron_script_process(proc, expected_create_time=expected_create_time)
    _sweep_tagged_cron_script_processes(run_token)
...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    try:
        ...
        env[_CRON_SCRIPT_RUN_TOKEN_ENV] = run_token
        ...
        process_create_time = _snapshot_process_create_time(proc)
...
def _preflight_check_delivery(job: dict) -> Optional[str]:
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            # A credentialless multiplex satellite delivers through the primary
            # gateway's adapter when an enabled route targets this profile.
...
def run_job(
    ...
    *,
    ...
    # Legacy or hand-edited agent jobs can bypass create/update validation.
    # Pause an empty job rather than waking an LLM with no instruction forever.
    ...
    _cron_session_id = (
        f"cron_{job_id}_{execution_id or job.get('execution_id') or uuid.uuid4().hex}"
    )
...
def tick(
    ...
    *,
    ...
    """
    # A process that is provably stale must not win the tick lock while a
    # separate fresh gateway is available to dispatch the same due jobs.
    ...
    try:
        ...
        # Workdirs are task-scoped, so all due jobs use the parallel pool.
