    ...
    try:
        ...
        def _heartbeat_run_claim_if_due():
            ...
            if not _is_oneshot or not _run_claim_owner:
                return
            # Local feature (recovery/retry): refresh the one-shot run_claim on a
            # dynamic interval derived from the job's inactivity timeout so a
            # legitimately long run is never reaped as a dead owner.
            try:
                from cron.jobs import _oneshot_run_claim_ttl_seconds

                _hb_interval = max(10.0, (_oneshot_run_claim_ttl_seconds() or 1800.0) / 3)
            except Exception:
                _hb_interval = _RUN_CLAIM_HEARTBEAT_SECONDS
            _mono = time.monotonic()
            if _mono - _last_claim_heartbeat < _hb_interval:
                return
...
def tick(
    ...
    *,
    ...
    try:
        ...
        # Local feature (recovery/retry): settle claimed recovery jobs before
        # the normal due scan. Recovery rows were marked "running" by
        # finish_recovery_job's claim path; each is re-eligible via
        # allow_recovery_rerun and runs without touching the base schedule.
        ...
        def _process_job(job: dict) -> bool:
            ...
            #
            # Local feature (recovery retry): a job flagged allow_recovery_rerun
            # is re-eligible for dispatch by the recovery scan even though the
            # durable fire claim is still live, so skip the claim-loss failure
            # path for those — they are re-running an already claimed delivery.
            _is_recovery_retry = bool(job.get("allow_recovery_rerun"))
            claimed = claim_job_for_fire(job["id"], return_job=True)
            if not claimed and not _is_recovery_retry:
                finish_execution(
                    job["execution_id"],
                    success=False,
